...
let parseContent(lines: string array) =
    let asScan (str : string) =
        let isX = str.[0] = 'x'
        let single, range = splitByFn ", " (fun p -> int p.[0].[2..], p.[1]) str
        let range1, range2 = splitByFn ".." (fun p -> int p.[0], int p.[1]) range.[2..]
        if isX then
            (single, range1), (single, range2)
        else
            (range1, single), (range2, single)
    
    lines |> Seq.map asScan
...
let reachedTiles =
    getCounts >> (fun counts -> Map.find StillWater counts + Map.find MovingWater counts)

let execute =
    let path = "day17/day17_input.txt"
    let content = LocalHelper.GetLinesFromFile path
    let counts = parseContent content
    reachedTiles counts