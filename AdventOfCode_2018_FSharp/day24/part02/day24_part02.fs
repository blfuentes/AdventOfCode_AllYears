...
let parseWeaknessesAndImmunities str =
    if str = "" then
        ...
        let parseSection (section : string) =
            ...
            isWeakness, section.[typeOffset..] |> splitByFn ", " (Array.map parseDamageType)
        let sections = str.Trim([| '('; ')'; ' ' |]) |> splitByFn "; " (Array.map parseSection)
...
let execute =
    ...
    inmuneSystemSize(parseGroups content)